...
module.exports = (prayerTimesCollection) => {
  ...
  router.put("/update", async (req, res) => {
    ...
    try {
      ...
      // Separate Jumu'ah updates from daily prayer updates
      const jumuahUpdates = {};
      const dailyUpdates = {};

      for (const [key, value] of Object.entries(updates)) {
        if (key.startsWith("jumuah")) {
          // Store Jumu'ah updates separately
          jumuahUpdates[key] = value;
        } else {
          // Handle daily prayer updates (fajr.start, sunrise, etc.)
          if (key.includes(".")) {
            // Handle nested fields like "fajr.start"
            const keys = key.split(".");
            let updatePath = `${month}.${dayIndex}`;
      ...
      // Only add Jumu'ah updates to the root object, NOT to the specific day
      if (Object.keys(jumuahUpdates).length > 0) {
        // Map the incoming field names to the correct nested structure
        if (jumuahUpdates.jumuahSummer1)
          updateObj["jumuah.summer.first"] = jumuahUpdates.jumuahSummer1;
        if (jumuahUpdates.jumuahSummer2)
          updateObj["jumuah.summer.second"] = jumuahUpdates.jumuahSummer2;
        if (jumuahUpdates.jumuahSummer3)
          updateObj["jumuah.summer.third"] = jumuahUpdates.jumuahSummer3;
        if (jumuahUpdates.jumuahWinter1)
          updateObj["jumuah.winter.first"] = jumuahUpdates.jumuahWinter1;
        if (jumuahUpdates.jumuahWinter2)
          updateObj["jumuah.winter.second"] = jumuahUpdates.jumuahWinter2;
        if (jumuahUpdates.jumuahWinter3)
          updateObj["jumuah.winter.third"] = jumuahUpdates.jumuahWinter3;
      }
      ...
      res.send({
        message: "Prayer time updated",
        result,
        updatesApplied: {
          dailyUpdates:
            Object.keys(dailyUpdates).length > 0 ? dailyUpdates : null,
          jumuahUpdates:
            Object.keys(jumuahUpdates).length > 0 ? jumuahUpdates : null,
        },
      });
    } catch (err) {
      console.error("Update error:", err);
